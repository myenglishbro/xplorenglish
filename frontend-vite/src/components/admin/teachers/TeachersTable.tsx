 ...
 * profiles.status -- ver decisión documentada en pages/admin/DocentesListPage.tsx. profiles.status
...
export function TeachersTable({ items }: { items: TeacherListItem[] }) {
  ...
  return <DataTable columns={columns} rows={items} onRowClick={(row) => navigate(`/admin/docentes/${row.id}`)} />;
}
