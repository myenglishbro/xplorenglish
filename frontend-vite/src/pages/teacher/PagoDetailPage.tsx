...
import { usePayrollPeriodDetail } from "@/features/payroll/hooks";
...
export function TeacherPagoDetailPage() {
  const { id } = useParams<{ id: string }>();
  const periodId = Number(id);
  const { data: period, isLoading, isError } = usePayrollPeriodDetail(periodId);

  if (!Number.isFinite(periodId)) return <Alert tone="danger">Periodo inválido.</Alert>;

  if (isLoading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", padding: "var(--space-6) 0" }}>
        <Spinner size={28} label="Cargando periodo…" />
      </div>
    );
  }

  if (isError || !period) {
    return <Alert tone="danger">No pudimos cargar este periodo (o no tienes acceso).</Alert>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "var(--space-5)" }}>
        ...
        <Card header={cardTitle("Recibo")}>
          <TeacherReceiptSection periodId={period.id} status={period.status} receipt={period.receipt} />
        </Card>
      </div>
