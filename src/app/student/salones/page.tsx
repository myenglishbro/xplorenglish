...
export default async function StudentSalonesPage() {
  const user = await getAuthUser();
  const supabase = createClient();
  const classrooms = user ? await listMyClassroomsAsStudent(supabase, user.id) : [];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
      ...
      {classrooms.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "var(--space-4)" }}>
          {classrooms.map((c) => (
            <Link key={c.id} href={`/student/salones/${c.id}`} style={{ textDecoration: "none" }}>
