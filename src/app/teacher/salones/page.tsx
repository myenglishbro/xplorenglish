...
export default async function TeacherSalonesPage() {
  const user = await getAuthUser();
  const supabase = createClient();
  const classrooms = user ? await listMyClassroomsAsTeacher(supabase, user.id) : [];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-5)" }}>
      ...
      {classrooms.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "var(--space-4)" }}>
          {classrooms.map((c) => (
            <Link key={c.id} href={`/teacher/salones/${c.id}`} style={{ textDecoration: "none" }}>
