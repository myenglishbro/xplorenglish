...
import { addClassroomStudentAction, removeClassroomStudentAction } from "@/server/admin/classrooms/actions";
...
export function ClassroomStudentsPanel({ classroomId, students, assignableStudents }: ClassroomStudentsPanelProps) {
  ...
  async function handleAdd(studentId: string) {
    ...
    setPending(true);
    setError(undefined);
    const result = await addClassroomStudentAction(classroomId, studentId);
  ...
  async function handleRemove(studentId: string) {
    ...
    setPending(true);
    setError(undefined);
    const result = await removeClassroomStudentAction(classroomId, studentId);
    if (result.error) setError(result.error);
    else router.refresh();
    setPending(false);
  }
