...
/** Navegación por Link plano (sin "use client"): igual criterio que Pagination.tsx -- la semana
 * vive en la URL (?week=YYYY-MM-DD, lunes de esa semana), el Server Component la resuelve con
 * getWeekRangeInLima. "Hoy" simplemente omite el parámetro. */
export function WeekNav({ weekStartIso, basePath, searchParams }: WeekNavProps) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)", flexWrap: "wrap" }}>
      <Link href={buildHref(basePath, searchParams, prevWeekParam)} style={{ textDecoration: "none" }}>
        <Button variant="secondary" size="sm" icon="caret-left">
          Semana anterior
        </Button>
      </Link>
      <Link href={buildHref(basePath, searchParams)} style={{ textDecoration: "none" }}>
        <Button variant="secondary" size="sm">
          Hoy
        </Button>
      </Link>
      <Link href={buildHref(basePath, searchParams, nextWeekParam)} style={{ textDecoration: "none" }}>
