...
import { updateMyTeacherProfileAction, type UpdateMyTeacherProfileActionState } from "@/server/teacher/profile/actions";
...
/** Solo nombre/apellido/teléfono -- DNI y email se muestran aparte, como texto, en el Server
 * Component que renderiza este form (nunca como inputs de este formulario). */
export function MyTeacherProfileForm({ profile }: MyTeacherProfileFormProps) {
  const [pending, setPending] = React.useState(false);
  const [state, setState] = React.useState<UpdateMyTeacherProfileActionState>({});
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updateMyTeacherProfileAction(formData);
    setState(result);
    setSaved(!result.error && !result.fieldErrors);
    setPending(false);
    // La action revalida /teacher/perfil; el RSC actualiza perfil y shell.
  }
